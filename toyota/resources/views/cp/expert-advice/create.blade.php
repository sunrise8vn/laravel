                ...
                <div class="box-body">
            		<div class="col-md-3">
                        <div class="form-group">
                            <button type="button" class="btn btn-block btn-success btn-flat" onclick="openPage('/cp/expert-advice')">Quay về danh sách</button>
                        </div>
                    </div>
	            	<form action="cp/expert-advice/create" method="POST" enctype="multipart/form-data">
	                    @csrf
                        <div class="col-md-12">
                            <div class="form-group">
                              	<label>Nhóm lời khuyên chuyên gia</label>
                              	<select class="form-control" name='expertAdviceGroup'>
                                  	@foreach($expertAdviceGroup as $exAdGrp)
                                      	<option value="{{$exAdGrp->id}}">{{$exAdGrp->title}}</option>
                                  	@endforeach
                              	</select>
                          </div>
                        </div>
