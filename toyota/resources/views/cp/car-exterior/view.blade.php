                	...
                	<div class="col-md-12">
	            		<div class="col-md-3">
	                        <div class="form-group">
	                            <button type="button" class="btn btn-block btn-success btn-flat" onclick="openPage('/cp/car-exterior')">Quay về danh sách</button>
	                        </div>
	                    </div>
	                </div>
	                <div class="col-md-12">
	                	<form action="cp/car-exterior/create/{{$cars->id}}" method="post" enctype="multipart/form-data">
	                		@csrf
                            <div class="col-md-6">
                                <div class="form-group">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label>Mô tả</label>
                                    <textarea name="description" maxlength="1000" style="width: 100%; height: 70px; padding: 10px;"></textarea>
                                </div>
                            </div>
                        ...
                        <div class="col-md-12">
	                        <div class="table-responsive">
	                            <table id="example1" class="table table-bordered table-striped dataTable">
	                                ...
	                                <tbody>
	                                    @foreach($carExteriors as $ext)
	                                        <tr id="row{{$ext->id}}" >
	                                            <td class="center" style="max-width: 150px;">{{$ext->title}}</td>
	                                            <td class="center" style="max-width: 450px;">{{$ext->description}}</td>
	                                            <td class="center">
	                                                <div style="width: 100%; text-align: center;">
	                                                    <img class="lazy zoom" data-car="{{$cars->id}}" data-avatar="{{$ext->avatar}}" onclick="zoomAvatar();" alt="{{$ext->avatar}}" src="/data/car/exterior/{{$cars->id}}/{{$ext->avatar}}" style="max-width: 100%; max-height: 105px;">
	                                                </div>
	                                            </td>
                                                <td class="center"><a class="chooseExterior" href="#editModal" data-id="{{$ext->id}}" data-car="{{$cars->id}}" data-title="{{$ext->title}}" data-avatar="{{$ext->avatar}}" data-description="{{$ext->description}}" data-toggle="modal"><i class="fa fa-pencil-square-o fa-fw"></i>Sửa</a></td>
	                                            <td class="center">
                                                    <a onclick="deleteConfirm({{$ext->id}});"><i class="fa fa-trash-o fa-fw"></i>Xóa</a>
                                                </td>
	                                        </tr>
	                                    @endforeach
	                                </tbody>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="col-md-12" style="text-align: center; padding: 5px;">
                    <h3>Cập nhật thông tin sản phẩm ngoại thất</h3>
                </div>
                ...
                <div>
                    ...
                    <div class="tab-content">
                        <form action="{{ route('exterior.edit') }}" method="post" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-group">
                                    <label>Mô tả</label>
                                    <textarea name="descriptionModal" maxlength="1000" style="width: 100%; height: 150px; padding: 10px;"></textarea>
                                </div>
                            </div>
...
@section('script')
    ...
    <script type="text/javascript">
        $('.chooseExterior').click(function() {
            ...
            $('#avatar_tempModal').attr('src', '/data/car/exterior/' + car_id + '/' + avatar);
        });
        ...
        function zoomAvatar(carid, avatar) {
            Swal.fire({
              imageUrl: '/data/car/exterior/' + carid + '/' + avatar,
            })
        }
        ...
        function deleteConfirm(id) {
            Swal.fire({
              ...
              if (result.value) {
                $.ajax({
                    type: "post",
                    url: "/api/cp/car-exterior/delete",
                    data : {
                        id : id,
                    },
                    success() {
