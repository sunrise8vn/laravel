...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Cập nhật bài viết chương trình kiểm tra miễn phí
      </h1>
      {{-- <ol class="breadcrumb">
                	...
                	<div class="row">
	            		<div class="col-md-3">
	                        <div class="form-group">
	                            <button type="button" class="btn btn-block btn-success btn-flat" onclick="openPage('{{ route('service.free-test-program.getList') }}')">Quay về danh sách</button>
	                        </div>
	                    </div>
                    </div>
                    <div class="row">
		            	<form action="{{ route('service.free-test-program.postEdit', $freeTestProgram->id) }}" method="POST" enctype="multipart/form-data">
		                    @csrf
	                        <div class="col-md-12">
	                            <div class="form-group">
	                                <label>Tiêu đề</label>
	                                <input type="text" class="form-control" name="title" required value="{{$freeTestProgram->title}}" placeholder="Nhập tiêu đề" />
	                            </div>
	                        </div>
	                        <div class="col-md-12">
	                            <div class="form-group">
	                                <label>Nội dung</label>
	                                <textarea name="content" style="width: 100%; height: 800px;">{!!$freeTestProgram->content!!}</textarea>
	                            </div>
	                        </div>
