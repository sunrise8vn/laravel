...
<div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Chỉnh sửa bài viết dịch vụ bảo dưỡng
      </h1>
      {{-- <ol class="breadcrumb">
                	...
                	<div class="row">
	            		<div class="col-md-3">
	                        <div class="form-group">
	                            <button type="button" class="btn btn-block btn-success btn-flat" onclick="openPage('{{ route('service.maintenance-service.getList') }}')">Quay về danh sách</button>
	                        </div>
	                    </div>
                    </div>
                    <div class="row">
		            	<form action="{{ route('service.maintenance-service.postEdit', $maintenanceService->id) }}" method="POST" enctype="multipart/form-data">
		                    @csrf

	                        <div class="col-md-12">
	                            <div class="form-group">
	                                <label>Tiêu đề</label>
	                                <input type="text" class="form-control" name="title" value="{{$maintenanceService->title}}" required placeholder="Nhập tiêu đề" />
	                            </div>
	                        </div>
	                        <div class="col-md-12">
	                            <div class="form-group">
	                                <label>Nội dung</label>
	                                <textarea name="content" style="width: 100%; height: 800px;">{!!$maintenanceService->content!!}</textarea>
	                            </div>
	                        </div>
